...
let perform_fixed_move (p : t) (m : Move.Fixed_move.Super.t) : t =
  m |> Move.Fixed_move.G.to_move |> perform_move p

let perform_fixed_move_list (p : t) (ls : Move.Fixed_move.Super.t list) : t =
  ls
  |> List.fold ~init:p ~f:(fun p m -> m |> Move.Fixed_move.G.to_move |> perform_move p)
