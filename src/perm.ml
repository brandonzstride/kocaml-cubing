...
let is_identity (p : t) : bool =
  List.for_all Cubie.all ~f:(fun cubie ->
    0 = Cubie_with_orientation.compare (Cubie_with_orientation.of_cubie cubie) (p cubie)
    )
...
let to_corners_list (p : t) : Cubie_with_orientation.Corner.t list =
...
let map_edges (p : t) (ls : Cubie.Edge.t list) : Cubie_with_orientation.Edge.t list =
  let open List.Let_syntax in
  let%map x = ls in
...
let to_edges_list (p : t) : Cubie_with_orientation.Edge.t list =
  map_edges p Cubie.Edge.all

let to_ud_edges_list (p : t) : Cubie_with_orientation.Edge.t list =
  map_edges p Cubie.Edge.all_ud_edges

let to_ud_slice_edges_list (p : t) : Cubie_with_orientation.Edge.t list =
  map_edges p Cubie.Edge.all_ud_slice_edges

let pp (p : t) =
  let pp c = Cubie.pp c ^ " -> " ^ Cubie_with_orientation.pp (p c) in
  Cubie.all
  |> List.map ~f:pp
