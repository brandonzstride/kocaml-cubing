...
let id = Cubie_with_orientation.of_cubie

let equal (m1 : t) (m2 : t) : bool =
  List.for_all Cubie.all ~f:(fun x -> Cubie_with_orientation.compare (m1 x) (m2 x) = 0)
...
let ( * ) a b = 
  ...
  | Cubie.Edge _ as x ->
    ...
    let bx = b x |> Cubie_with_orientation.edge_exn in
    let abx = a (Cubie.Edge bx.e) |> Cubie_with_orientation.edge_exn in
    Cubie_with_orientation.Edge Cubie_with_orientation.Edge.{ e = abx.e ; o = Modular_int.Z2.(bx.o + abx.o) }
  | Cubie.Corner _ as x ->
    (** Safe to use corner_exn if the move is well-formed and maps corners to corners *)
    let bx = b x |> Cubie_with_orientation.corner_exn in
    let abx = a (Cubie.Corner bx.c) |> Cubie_with_orientation.corner_exn in
    Cubie_with_orientation.Corner Cubie_with_orientation.Corner.{ c = abx.c ; o = Modular_int.Z3.(bx.o + abx.o) }
    ...
    let to_move = 
      ...
      | R -> begin function
        ...
        | Corner c -> 
          ...
          in Cubie_with_orientation.Corner { c = c' ; o = Z3.of_int o }
        end
      | F -> begin function
        | Edge e -> 
