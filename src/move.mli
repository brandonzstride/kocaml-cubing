...
(*
  I may prefer instead to have only the generators and an "all_multiples" function
  that sends the generator to all possible non-identity multiples of itself, i.e.
  all moves that are generated by that generator alone.

  Maybe this would be `all_moves : t -> All_fixed_move.t`, but then I could rename
  All_fixed_move to be a little better. The issue here is that it doesn't help restrict
  the `perform_fixed_move` in the coordinates but, we'd have to do `perform_generator`,
  which might be okay, but it requires that user to perform more complex moves. And also
  that might run into issue when I reintroduce reflection because generators don't always
  have a generator symmetric move.
*)
