...
module S =
  struct
    ...
    *)
    ...
    let on_fixed_move (s : t) (m : Move.Fixed.Super.t) : Move.Fixed.Super.t = 
      let m' = Move.(to_move s * Move.Fixed.G.to_move m * to_move (inverse s)) in
      List.find Move.Fixed.G.all ~f:(fun a -> Move.equal (Move.Fixed.G.to_move a) m')
      |> function
        | Some m -> m
